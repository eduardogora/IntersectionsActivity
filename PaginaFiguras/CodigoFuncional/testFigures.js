...
function draw(segms) {
  ...
  if (canvas.getContext) {
    ...
    segms.forEach(segm => {
      ...
      //console.log("Tam: ", tam);
      ctx.moveTo(segm.puntoI.x, segm.puntoI.y);
      ctx.lineTo(segm.puntoF.x, segm.puntoF.y);
...
function start(){
  ...
  for(var i = 0; i < figuras.length; i++){
    ...
    for(var j = 0; j < figuras[i].aristas.length; j++){
      var s = getCords(figuras[i], figuras[i].aristas[j].origen, figuras[i].aristas[j].sigue);
